    ...
    <div className="flex min-h-screen bg-black">
      <div className={`${
        ...
        <div className={`p-4 flex ${isSidebarOpen ? 'gap-2' : 'justify-center'} items-center`}>
          ...
          {isSidebarOpen && <img src={leanLearnLogo} alt="LeanLearn" className="h-8 pl-12" />}
        </div>
        ...
        <div className="flex justify-between items-center mb-8">
          <div className="relative inline-block">
            <select 
              ...
              className="bg-[#1A1A1A] text-white px-4 py-2 rounded-md focus:outline-none appearance-none pr-10"
            >
              {[...Array(12)].map((_, i) => (
          ...
          <button onClick={() => navigate('/create-quiz')} className="flex items-center">
            ...
            {expandDraft && (
              <div className="space-y-4">
                {draftQuizzes.map((quiz) => (
                  <div 
                    key={quiz.id}
                    className="bg-[#1A1A1A] p-4 rounded-lg flex items-center justify-between text-white hover:bg-[#2A2A2A] transition-colors cursor-pointer relative group border border-transparent hover:border-gray-800"
                  >
